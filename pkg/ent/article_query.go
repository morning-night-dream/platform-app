...
type ArticleQuery struct {
	...
	withReadArticles *ReadArticleQuery
...
func (aq *ArticleQuery) QueryReadArticles() *ReadArticleQuery {
	query := (&ReadArticleClient{config: aq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (aq *ArticleQuery) Clone() *ArticleQuery {
	...
	return &ArticleQuery{
		...
		withReadArticles: aq.withReadArticles.Clone(),
...
// WithReadArticles tells the query-builder to eager-load the nodes that are connected to
// the "read_articles" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *ArticleQuery) WithReadArticles(opts ...func(*ReadArticleQuery)) *ArticleQuery {
	query := (&ReadArticleClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withReadArticles = query
	return aq
}
...
func (aq *ArticleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Article, error) {
	...
	if query := aq.withTags; query != nil {
		if err := aq.loadTags(ctx, query, nodes,
			func(n *Article) { n.Edges.Tags = []*ArticleTag{} },
			func(n *Article, e *ArticleTag) { n.Edges.Tags = append(n.Edges.Tags, e) }); err != nil {
			return nil, err
		}
	}
	if query := aq.withReadArticles; query != nil {
		if err := aq.loadReadArticles(ctx, query, nodes,
			func(n *Article) { n.Edges.ReadArticles = []*ReadArticle{} },
			func(n *Article, e *ReadArticle) { n.Edges.ReadArticles = append(n.Edges.ReadArticles, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *ArticleQuery) loadTags(ctx context.Context, query *ArticleTagQuery, nodes []*Article, init func(*Article), assign func(*Article, *ArticleTag)) error {
...
func (aq *ArticleQuery) loadReadArticles(ctx context.Context, query *ReadArticleQuery, nodes []*Article, init func(*Article), assign func(*Article, *ReadArticle)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Article)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
